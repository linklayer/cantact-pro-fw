		...
		usb_device_control_request_struct_t *control_request)
{
    ...
    if ((setup.bmRequestType & 0x80) == 0 && (control_request->buffer == NULL)) {
    ...
    switch (setup.bRequest)
    {
    	...
    	case GS_USB_BREQ_HOST_FORMAT:
    		memcpy((uint8_t *)&can_dev.host_config, control_request->buffer, sizeof(can_dev.host_config));
    		break;
                                    ...
                                    void *callbackParam)
{
    ...
    else if ((message->buffer != NULL) || ((message->buffer == NULL) && (message->length == 0)))
	{
		...
		USB_DeviceRecvRequest(handle, GSUSB_ENDPOINT_OUT, s_currRecvBuf, s_usbBulkMaxPacketSize);
                                     ...
                                     void *callbackParam)
{
	...
	//tx_enqueue(frame);
	can_send(frame->channel, frame->echo_id,
			frame->can_id,
			frame->can_dlc,
			frame->data);
...
usb_status_t USB_DeviceCallback(usb_device_handle handle, uint32_t event, void *param)
{
    ...
    (defined(USB_DEVICE_CONFIG_LPCIP3511HS) && (USB_DEVICE_CONFIG_LPCIP3511HS > 0U))
            ...
            if (kStatus_USB_Success ==
                USB_DeviceGetStatus(s_usb_can.deviceHandle, kUSB_DeviceStatusSpeed, &s_usb_can.speed))
            {
        ...
        case kUSB_DeviceEventVendorRequest:
        	USB_DeviceProcessVendorRequest(handle, (usb_device_control_request_struct_t *)param);
        	break;
        default:
        	usb_echo("idk %d\n", event);
            break;
...
void gs_usb_init()
{
    ...
    if (kStatus_USB_Success != USB_DeviceClassInit(CONTROLLER_ID, &s_usbClassConfigList, &s_usb_can.deviceHandle))
    {
